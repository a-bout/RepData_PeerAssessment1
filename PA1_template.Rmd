...
```
`r maxNumberOfSteps` interval contains the maximum number of steps.

## Imputing missing values
```{r}
totalNa <- sum(is.na(data)) 
```
The total count of missing values is `r totalNa`
...
```
The mean total number of steps per day after imputing missing values is `r stepMeanFull`.

The median total number of steps per day after imputing missing values is `r stepMedianFull`
